...
export const createCourseOffering = async (
  ...
  if (duplicate) throw new AppError(409, "This course offering already exists.");
...
export const getCourseOfferings = async (query: GetCourseOfferingsQuery) =>
  prisma.courseOffering.findMany({
    ...
    include,
    orderBy: [{ year: "desc" }, { semester: "desc" }, { course: { code: "asc" } }],
  });

export const getCourseOfferingById = async (id: string) => {
  const offering = await prisma.courseOffering.findUnique({ where: { id }, include });
...
export const updateCourseOffering = async (
  ...
  const capacity = payload.capacity === undefined ? existing.capacity : payload.capacity;
  if (capacity !== null && capacity !== undefined && capacity < existing._count.enrollments) {
  ...
  return prisma.courseOffering.update({ where: { id }, data: payload, include });
};

export const deleteCourseOffering = async (userId: string, id: string) => {
  const existing = await prisma.courseOffering.findUnique({
    where: { id },
    include: { course: true, _count: { select: { enrollments: true, classSession: true, exam: true } } },
  });
  if (!existing) throw new AppError(404, "Course offering not found.");
  await ensureManager(userId, existing.course.departmentId);
  if (existing._count.enrollments || existing._count.classSession || existing._count.exam) {
    throw new AppError(409, "Cannot delete an offering with related academic records.");
  }
