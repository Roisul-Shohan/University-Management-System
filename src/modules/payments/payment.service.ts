...
import {
  sendAdmissionWelcomeEmail,
  sendCourseRegistrationEmail,
  sendSemesterRegistrationEmail,
} from "../../utils/email.js";
import {
  AdmissionStatus,
  EnrollmentStatus,
...
const finalizeSuccessfulPayment = async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    switch (currentTransaction.type) {
      ...
      case TransactionType.COURSE_REGISTRATION: {
        if (!currentTransaction.studentSemesterId) {
          throw new AppError(
            409,
            "Student semester information is missing for this transaction.",
  ...
  if (
    ...
    const student = await createStudentFromConfirmedAdmission({
...
export const executePayment = async ({
  ...
  switch (transaction.type) {
    ...
    case TransactionType.SEMESTER_REGISTRATION:
      if (
        transaction.studentSemester?.status !== StudentSemesterStatus.PENDING
      ) {
        throw new AppError(
          409,
          "Semester is not in a valid state for payment completion.",
        );
      }
      break;

    case TransactionType.COURSE_REGISTRATION:
      // Enrollment status is ENROLLED natively. No specific pre-check status needed here as it was checked during initiation.
      break;
