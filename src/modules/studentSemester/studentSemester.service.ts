...
export const initiateSemesterPayment = async ({
  ...
  if (studentSemester.student.id !== userId) {
    throw new AppError(
      403,
      "You are not authorized to make a payment for this semester.",
    );
  }
  ...
  if (studentSemester.status !== StudentSemesterStatus.PENDING) {
    throw new AppError(
      409,
      `Cannot initiate payment. Semester status is ${studentSemester.status}.`,
    );
  }
  ...
  const transaction = await prisma.$transaction(async (tx) => {
    ...
    if (existingSuccessful) {
      throw new AppError(
        409,
        "Semester registration payment has already been completed.",
      );
    }
    ...
    if (existingPending) {
      throw new AppError(
        409,
        "There is already a pending payment for this semester.",
      );
    }
...
export const initiateCourseRegistrationPayment = async ({
  studentSemesterId,
  userId,
}: IInitiateSemesterPayment) => {
  const studentSemester = await prisma.studentSemester.findUnique({
  ...
  if (studentSemester.courseEnrollments.length === 0) {
    throw new AppError(
      400,
  ...
  if (studentSemester.status !== StudentSemesterStatus.REGISTERED) {
    throw new AppError(
      409,
      "Semester registration must be completed before course registration.",
    );
  }

  const activeFee = studentSemester.student.program.creditFees[0];
  if (!activeFee) {
    throw new AppError(404, "No active per-credit fee found for your program.");
  }
  ...
  const totalCredits = studentSemester.courseEnrollments.reduce(
    (sum, enrollment) => {
      return sum + (enrollment.courseOffering.course.credits || 0);
    },
    0,
  );
  ...
  if (totalCredits === 0) {
    throw new AppError(
      400,
      "Total enrolled credits is zero. Registration payment cannot be processed.",
    );
  }

  const feeAmount = new Prisma.Decimal(activeFee.amount).mul(totalCredits);
  ...
  const transaction = await prisma.$transaction(async (tx) => {
    ...
    if (existingSuccessful) {
      throw new AppError(
        409,
        "Course registration payment has already been completed.",
      );
    }
    ...
    if (existingPending) {
      throw new AppError(
        409,
        "There is already a pending payment for course registration.",
      );
    }
